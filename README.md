...
1. **Data cleaning** — handle missing `TotalCharges`, encode target
2. **EDA** — churn distribution, tenure vs churn, monthly charges vs churn (saved as plots)
3. **Feature engineering** — label encoding of categorical variables
4. **Class imbalance handling** — SMOTE oversampling on the training set (raw churn rate 26.6% → balanced 50% for training)
5. **Modeling** — Logistic Regression, Random Forest, and XGBoost (tuned via GridSearchCV)
6. **Evaluation** — accuracy, precision, recall, F1, ROC-AUC, confusion matrices
7. **Feature importance** — visualized for Random Forest and XGBoost
